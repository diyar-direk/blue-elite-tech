...
const Courses = () => {
  ...
  const handleSearch = (e) => {
    ...
    if (inpValue === "") {
      ...
      const filteredData = data.filter(
        (item) =>
          item.headline.en.toLowerCase().includes(inpValue) ||
          item.summary.en.toLowerCase().includes(inpValue)
      );
  ...
  const tableData = searchData.map((item, index) => {
    return (
      <tr key={item._id}>
        <td>{index + 1}</td>
        <td className="align-left">{item.headline.en}</td>
        <td>{item.summary.en}</td>
        <td>
